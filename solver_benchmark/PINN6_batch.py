...
os.environ["XLA_PYTHON_CLIENT_PREALLOCATE"] = (
    "false"  # add this for memory pre-allocation
)
...
class PINN:
    def __init__(self, layer_sizes, cls_cp, bc_ic_loss_weight, x_min, x_max, eta, d):
        """ """
    ...
    def rhs_function(self, x):
        ...
        ## Call variables
        x, y, z, t, P = x[0], x[1], x[2], x[3], x[5]
        eta, d = self.eta, self.d
...
# --------------------- Global setup -----------------------------
gpu_idx = 6  # set which GPU to run on Athena
...
# --------------------- Training setup ----------------------------
nlayers = 3
nneurons = 60  # 20, the smaller the better training

num_epochs = 100_000
batch_size = 2_000
learning_rate = 2e-3
stopping_criteria = 2e-2
bc_ic_loss_weight = 1.0
dim = 6  # x,y,z,t,k,P
var = 1
eta, d = 0.102, 0.0304

n_cp = 1_000_000  # number of collocation points
...
if method == "PINN":
    ...
    x_min = np.array([0, 0, 0, 0, 1, 100], dtype=np.double)  # x,y,z,t,k,P
    x_max = np.array([10, 5, 2.5, 0.04, 4, 200], dtype=np.double)  # x,y,z,t,k,P
    ...
    pinn = PINN(layer_sizes,cp,bc_ic_loss_weight,jnp.array(x_min),jnp.array(x_max),eta,d)
...
P = 101
X, Y = np.meshgrid(x, y)
# k, P, R, eta, d = 3.5, 145, 1.3, 0.04, 0.03
times = np.linspace(0, 0.04, dim)  # time range
