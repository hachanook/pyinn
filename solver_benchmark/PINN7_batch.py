...
class PINN:
    def __init__(self, layer_sizes, cls_cp, bc_ic_loss_weight, x_min, x_max, d):
        """ """
    ...
    def rhs_function(self, x):
        ...
        ## Call variables
        x, y, z, t, P, eta = x[0], x[1], x[2], x[3], x[5], x[6]
        d = self.d
...
# --------------------- Training setup ----------------------------
nlayers = 3
nneurons = 70  # 20, the smaller the better training
...
dim = 7  # x,y,z,t,k,P,eta
var = 1
d = 0.0304

n_cp = 10_000_000  # number of collocation points
...
if method == "PINN":
    ...
    x_min = np.array([0, 0, 0, 0, 1, 100, 0.10], dtype=np.double)  # x,y,z,t,k,P,eta
    x_max = np.array([10, 5, 2.5, 0.04, 4, 200, 0.20], dtype=np.double)  # x,y,z,t,k,P,eta
    ...
    pinn = PINN(layer_sizes,cp,bc_ic_loss_weight,jnp.array(x_min),jnp.array(x_max),d)
