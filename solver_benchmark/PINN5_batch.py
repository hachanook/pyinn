...
class PINN:
    def __init__(self, layer_sizes, cls_cp, bc_ic_loss_weight, x_min, x_max, P, eta, d):
        """ """
...
# --------------------- Training setup ----------------------------
nlayers = 3
nneurons = 50  # dim * 10, the smaller the better training
...
dim = 5  # x,y,z,t,k
var = 1
P, eta, d = 101, 0.102, 0.0304

n_cp = 100_000  # number of collocation points
...
if method == "PINN":
    ...
    x_min = np.array([0, 0, 0, 0, 1], dtype=np.double)  # x,y,z,t,k
    x_max = np.array([10, 5, 2.5, 0.04, 4], dtype=np.double)  # x,y,z,t,k
    ...
    pinn = PINN(layer_sizes,cp,bc_ic_loss_weight,jnp.array(x_min),jnp.array(x_max),P,eta,d)
