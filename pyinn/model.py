...
class INN_linear:
    ...
    vv_forward = jax.vmap(v_forward, in_axes=(None,None, 0)) # returns (ndata,ndata, var)
    # vp_forward = jax.vmap(forward, in_axes=(None,0, None)) # returns (J, var) # for ALS
    # v_vp_forward = jax.vmap(vp_forward, in_axes=(None,None, 0)) # returns (ndata, J, var) # for ALS, to be debugged
    ...
    vp_forward_als = jax.vmap(forward_als, in_axes=(None,0, None)) # returns (J, M, var) # for ALS
    v_vp_forward_als = jax.vmap(vp_forward_als, in_axes=(None,None, 0)) # returns (ndata, J, M, var) # for ALS, to be debugged
